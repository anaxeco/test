...
namespace _04_interactions_framework.Modules
{
    ...
    public class UtilityModule : InteractionModuleBase<SocketInteractionContext>
    {
        ...
        [SlashCommand("roll", "schizo")]
        public async Task Roll()
        {
            string[] responses =
            {
                "You don't believe a word that was just said to you.",
                "A woman's voice sounds in your ear telling you a story about a bullfrog names Jeremiah.",
                "You feel insects crawling all over your skin.",
                "You're pretty sure this is a simulation.",
                "Five",
                "Six",
                "Seven",
                "Eight",
            ...
            var rand = new Random(); //connects to 'Random' object
            int number  = rand.Next(responses.Length); //pulls number from 'Random' object
            await RespondAsync(responses[number]); //responds with correlating number value
        }
        ...
        public async Task Echo(string echo, [Summary(description: "mention the user")] IUser bitches)
        {
            await RespondAsync(echo + bitches.Mention);
        }
